  ...
  await AsyncRetry(
    async (_, attemptNumber) => {
      try {
        ...
        const txStatus = await fallbackPeerRequest<
          { block_height: number; number_of_confirmations: number } | undefined
        >(`/tx/${bundleId}/status`, { returnOnAllFailure: true });
        ...
        if (!res) {
          logger.warn(`[verifyBundle] Falling back to direct download from gateway`);
          const gwBundleStream = await axios({ url: `https://arweave.net/${bundleId}/data`, responseType: "stream" });
          res = await processStream(gwBundleStream.data);
        }
    ...
    },
    {
      retries: 3,
      onRetry: (e, attempt) => {
        logger.error(`[verifyBundle] Error processing bundle ${bundleId} on attempt ${attempt} - ${fmtError(e)}`);
      },
    },
  );
}
