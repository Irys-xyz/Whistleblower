...
import { MAX_BUNDLE_AGE, MAX_TX_AGE, PRESERVE_INVALID } from "@utils/env";
import logger from "@logger";
import { dateToString } from "knex/lib/util/string";

export async function pruneOldTxs(maxAge = MAX_TX_AGE, preserveInvalid = PRESERVE_INVALID): Promise<void> {
...
export async function pruneOldBundles(maxAge = MAX_BUNDLE_AGE, preserveInvalid = PRESERVE_INVALID): Promise<void> {
  const threshold = new Date(Date.now() - maxAge);
